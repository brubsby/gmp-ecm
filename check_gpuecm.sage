...
def smallestGroupOrder(prime, param, sigma_0, test_B1, num_curves):
    ...
    for sigma in range(sigma_0, sigma_0 + num_curves):
        ...
        # B1 to find this factor is 2nd smallest if B2 = 100 * B1 finds large factor
        test = exp[-2] if len(exp) >= 2 and exp[-1] < exp[-2] * 100 else exp[-1]
        if test <= smallest:
            print ("\tFound by sigma: %d with B1=%d, B2=%d" % (sigma, test, exp[-1]))
            smallest = test
            found_at = sigma
    return found_at
